...
/* Minimum elapsed ms required before reporting idle percent */
#define LV_FREERTOS_IDLE_SAMPLE_PERIOD_MS 5000U
...
{
    static uint32_t last_sample_ts = 0;

    /* On first call start the sampling window */
    if(last_sample_ts == 0) {
        last_sample_ts = lv_tick_get();
        LV_LOG_WARN("Not enough time elapsed to provide idle percentage\r\n");
        return 0;
    }

    /* If the sampling window hasn't elapsed yet, report not enough time */
    // if(lv_tick_elaps(last_sample_ts) < LV_FREERTOS_IDLE_SAMPLE_PERIOD_MS) {
    //     LV_LOG_WARN("Not enough time elapsed to provide idle percentage\r\n");
	// 			printf("%d",lv_tick_elaps(last_sample_ts));
    //     return 0;
    // }

    if(globals->freertos_non_idle_time_sum + globals->freertos_idle_time_sum == 0) {
        LV_LOG_WARN("No activity recorded during sampling period");
        last_sample_ts = lv_tick_get();
        return 0;
    }
