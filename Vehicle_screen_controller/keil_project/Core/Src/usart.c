...
void MX_USART3_UART_Init(void)
{
  ...
  /*串口3环形缓冲区初始化*/
  circle_buf_init(&g_circle_buff,g_circle_buff_len,g_rx_buf);
...
{
	uint8_t state;
	while(huart3.gState != HAL_UART_STATE_READY); //??TX
	state = HAL_UART_Transmit(&huart3,pData,Size,Timeout);
	return state;
}
