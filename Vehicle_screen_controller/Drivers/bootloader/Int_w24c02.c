...
void Int_w24c02_write_bytes(uint8_t byte_addr, uint8_t *data, uint16_t len)
{
    ...
    else
    {
        ...
        {
            // 将当前页剩余的空间 写满
            HAL_I2C_Mem_Write(&hi2c1, W24C02_ADDR, start_page_addr, I2C_MEMADD_SIZE_8BIT, data + page_count * 16, page_remain_len, 1000);
        ...
        if (len != 0)
        {
            HAL_I2C_Mem_Write(&hi2c1, W24C02_ADDR, start_page_addr, I2C_MEMADD_SIZE_8BIT, data + page_count * 16, len, 1000);
        }
    }
}
